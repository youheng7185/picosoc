...
module	axil_ram #(
    ...
    always @(posedge S_AXI_ACLK) begin
        if (axil_write_ready) begin
            if (wskd_strb[0]) ram0[awskd_addr] <= wskd_data[7:0];
            if (wskd_strb[1]) ram1[awskd_addr] <= wskd_data[15:8];
            if (wskd_strb[2]) ram2[awskd_addr] <= wskd_data[23:16];
            if (wskd_strb[3]) ram3[awskd_addr] <= wskd_data[31:24];
        end
    end
	...
	begin
		axil_read_data <= { ram3[arskd_addr], ram2[arskd_addr],
                            ram1[arskd_addr], ram0[arskd_addr] };
